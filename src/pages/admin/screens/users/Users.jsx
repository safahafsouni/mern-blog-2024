...
const Users = () => {
  ...
  const { mutate: mutateUpdateUser, isLoading: updateUserIsLoading } =
    useMutation({
  ...
  const { mutate: mutateUpdateVerified, isLoading: updateVerifiedIsLoading } =
    useMutation({
  ...
  const handleVerifiedCheck = (event, userId) => {
    ...
    if (
      window.confirm(
        'Do you want to change the verification status of this user?'
      )
    ) {
      mutateUpdateVerified({ isVerified: event.target.checked, userId });
    } else {
      event.target.checked = initialCheckValue;
    }
  };
        ...
        <tr key={user._id}>
          ...
          <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
            <input
              ...
              disabled={updateVerifiedIsLoading}
            />
          </td>
